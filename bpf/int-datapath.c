...
int ingress(struct __sk_buff *skb)
{
    ...
    bpf_printk("Ingress, cpu=%d, port=%d, hash=%x", bpf_get_smp_processor_id(), skb->ifindex, hash);
...
int egress(struct __sk_buff *skb)
{
    ...
    __u32 hash = bpf_get_hash_recalc(skb);
    bpf_printk("Egress, cpu=%d, port=%d, hash=%x", bpf_get_smp_processor_id(), skb->ifindex, hash);
    ...
    bpf_printk("Hop latency=%llu, egress_port=%d", egress_timestamp-b->ingress_timestamp, skb->ifindex);
